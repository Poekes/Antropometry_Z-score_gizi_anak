...
void IbuSeeder::seed() {
    if (!UserModel::userExists("ibu_budi")) {
        ...
        UserModel::linkIbuToAnak(UserModel::getUserIdByUsername("ibu_budi"), "Budi");
    }
    if (!UserModel::userExists("ibu_siti")) {
        UserModel::registerUser(2, "ibu_siti", "123");
        UserModel::linkIbuToAnak(UserModel::getUserIdByUsername("ibu_siti"), "Siti");
    }
    if (!UserModel::userExists("ibu_andi")) {
        UserModel::registerUser(2, "ibu_andi", "123");
        UserModel::linkIbuToAnak(UserModel::getUserIdByUsername("ibu_andi"), "Andi");
    }
}
